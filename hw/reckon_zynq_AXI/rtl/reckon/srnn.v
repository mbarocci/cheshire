...
module srnn #(
    parameter N = 256,
    parameter M = 8,
	parameter WINP_MEM_PATH="",
	parameter WREC_MEM_PATH="",
	parameter WOUT_MEM_PATH="",
	parameter NEUR_MEM_PATH=""
)(
    // Global inputs   -------------------------------------
    input  wire                 CLK,
    input  wire                 RST,
    ...
    wire        [127:0] neur_data_o, neur_data_o2;
    ...
    wire        [127:0] winp_data_o2, wrec_data_o2, wout_data_o2;

    // Other ariables for logic generation
    genvar       i, j, k, n;
    integer      kk;

    assign r_state = state;
    //----------------------------------------------------------------------------------
    //    Sync barriers
    //----------------------------------------------------------------------------------
    ...
    .WINP_RAM_WIDTH (128),
    .WINP_RAM_DEPTH (4096),
    .WINP_INIT_FILE (WINP_MEM_PATH),

    .WREC_RAM_WIDTH (128),
    .WREC_RAM_DEPTH (4096),
    .WREC_INIT_FILE (WREC_MEM_PATH),

    .WOUT_RAM_WIDTH (128),
    .WOUT_RAM_DEPTH (512),
    .WOUT_INIT_FILE (WOUT_MEM_PATH),

    .NEUR_RAM_WIDTH (128),
    .NEUR_RAM_DEPTH (128),
    .NEUR_INIT_FILE (NEUR_MEM_PATH)

    ) weights_neur_RAM_new (

    .CLK(CLK),

    .winp_addr(winp_addr),
    .winp_din(winp_data_i),
    .winp_we(winp_we),
    .winp_cs(winp_cs),
    .winp_mask(winp_mask),
    .winp_dout(winp_data_o),

    .wrec_addr(wrec_addr),
    .wrec_din(wrec_data_i),
    .wrec_we(wrec_we),
    .wrec_cs(wrec_cs),
    .wrec_mask(wrec_mask),
    .wrec_dout(wrec_data_o),

    .wout_addr(wout_addr),
    .wout_din(wout_data_i),
    .wout_we(wout_we),
    .wout_cs(wout_cs),
    .wout_mask(wout_mask),
    .wout_dout(wout_data_o),

    .neur_addr(neur_addr),
    .neur_din(neur_data_i),
    .neur_we(neur_we),
    .neur_cs(neur_cs),
    .neur_mask(neur_mask),
    .neur_dout(neur_data_o)
