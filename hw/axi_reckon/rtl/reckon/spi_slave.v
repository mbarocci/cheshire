...
module spi_slave  #(
    ...
    output  reg        [  11:0] SPI_N_EPOCHS,
    output  reg        [  11:0] SPI_N_SAMPLES,
    output  reg        [  11:0] SPI_BATCH_SIZE,
    output  reg        [  11:0] SPI_LABEL_DELAY,
    output  reg        [  11:0] SPI_INFER_ACC_DELAY,
    ...
    // SPI counter // reset every 32 SCK cycles or 32*n, n = requested reads/writes on SRAM
    ...
    always @(negedge SCK, posedge RST_async)
        if (RST_async) begin
            ...
            spi_shift_reg_out     <= ( (spi_addr[30:28] == 3'b110) && (spi_addr[15:0] == 16'd1) ) ? {20'b0, infer_count} : SRNN_READBACK << 1;
            READ_NEUR             <= 1'b0;
            READ_ONEUR            <= 1'b0;
            READ_WINP             <= 1'b0;
            READ_WREC             <= 1'b0;
    ...
    //always @(posedge SCK)
    //    if      ( (spi_addr[31:28] == 4'b1110) && (spi_addr[15:0] == 16'd105) && &spi_cnt[4:0] && |spi_cnt[16:5])     SPI_NOISE_STR <= {spi_shift_reg_in[2:0], MOSI};
    ...
    always @(posedge SCK, posedge RST_async)
        if      (RST_async)                                                                              SPI_N_EPOCHS <= 12'd1;
        else if (~|spi_addr[31:28] && (spi_addr[15:0] == 16'd35 ) && &spi_cnt[4:0] && |spi_cnt[16:5])    SPI_N_EPOCHS <= {spi_shift_reg_in[11:0], MOSI};

    always @(posedge SCK, posedge RST_async)
        if      (RST_async)                                                                              SPI_N_SAMPLES <= 12'd1;
        else if (~|spi_addr[31:28] && (spi_addr[15:0] == 16'd36 ) && &spi_cnt[4:0] && |spi_cnt[16:5])    SPI_N_SAMPLES <= {spi_shift_reg_in[11:0], MOSI};

    always @(posedge SCK, posedge RST_async)
        if      (RST_async)                                                                              SPI_LABEL_DELAY <= 12'd2100;
        else if (~|spi_addr[31:28] && (spi_addr[15:0] == 16'd37 ) && &spi_cnt[4:0] && |spi_cnt[16:5])    SPI_LABEL_DELAY <= {spi_shift_reg_in[11:0], MOSI};

    always @(posedge SCK, posedge RST_async)
        if      (RST_async)                                                                              SPI_BATCH_SIZE <= 12'd1;
        else if (~|spi_addr[31:28] && (spi_addr[15:0] == 16'd38 ) && &spi_cnt[4:0] && |spi_cnt[16:5])    SPI_BATCH_SIZE <= {spi_shift_reg_in[11:0], MOSI};

    always @(posedge SCK, posedge RST_async)
        if      (RST_async)                                                                              SPI_INFER_ACC_DELAY <= 12'd1;
        else if (~|spi_addr[31:28] && (spi_addr[15:0] == 16'd39 ) && &spi_cnt[4:0] && |spi_cnt[16:5])    SPI_INFER_ACC_DELAY <= {spi_shift_reg_in[11:0], MOSI};
